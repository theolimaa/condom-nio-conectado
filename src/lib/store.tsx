...
const initialState: AppState = {
  ...
  selectedMonth: _now.getMonth(), // 0=Jan … 11=Dez — inicia no mês atual
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'UPDATE_CONDOMINIUM':
      return { ...state, condominiums: state.condominiums.map(c => c.id === action.payload.id ? action.payload : c) };
    ...
    case 'UPDATE_TENANT':
      return {
        ...
        apartments: state.apartments.map(a => {
          ...
          return { ...a, tenants: a.tenants.map(t => t.id === action.payload.tenant.id ? action.payload.tenant : t) };
        }),
      };
    case 'ADD_TENANT': {
      const tenant = action.payload.tenant;
      const newPayments = tenant.contract ? generatePaymentPeriods(action.payload.apartmentId, tenant.id, tenant.contract, _now.getFullYear()) : [];
      return {
        ...state,
        apartments: state.apartments.map(a => {
          if (a.id !== action.payload.apartmentId) return a;
          return { ...a, currentTenantId: tenant.id, tenants: [...a.tenants, tenant], payments: [...a.payments, ...newPayments] };
        }),
      };
    }
    case 'DELETE_TENANT':
      return {
        ...
        apartments: state.apartments.map(a => {
          ...
          return {
            ...a,
            currentTenantId: a.currentTenantId === action.payload.tenantId ? undefined : a.currentTenantId,
        ...
        apartments: state.apartments.map(a => {
          ...
          return {
            ...
            tenants: a.tenants.map(t => {
              ...
              return { ...t, isCurrent: false, movedOutAt: action.payload.endDate, contract: t.contract ? { ...t.contract, status: 'closed', endDate: action.payload.endDate } : t.contract };
            }),
          };
        }),
      };
    case 'ADD_DOCUMENT':
      return {
        ...
        apartments: state.apartments.map(a => {
          if (a.id !== action.payload.apartmentId) return a;
          return { ...a, tenants: a.tenants.map(t => { if (t.id !== action.payload.tenantId) return t; return { ...t, documents: [...t.documents, action.payload.document] }; }) };
        }),
      };
    case 'DELETE_DOCUMENT':
      return {
        ...
        apartments: state.apartments.map(a => {
          if (a.id !== action.payload.apartmentId) return a;
          return { ...a, tenants: a.tenants.map(t => { if (t.id !== action.payload.tenantId) return t; return { ...t, documents: t.documents.filter(d => d.id !== action.payload.documentId) }; }) };
        }),
      };
    case 'UPDATE_CONTRACT': {
      ...
      return {
        ...
        apartments: state.apartments.map(a => {
          if (a.id !== action.payload.apartmentId) return a;
          const updatedTenants = a.tenants.map(t => { if (t.contract?.id !== contract.id) return t; return { ...t, contract }; });
          const newPayments = generatePaymentPeriods(a.id, contract.id, contract, _now.getFullYear());
          const otherPayments = a.payments.filter(p => { const tenant = updatedTenants.find(t => t.id === p.tenantId); return !tenant || tenant.contract?.id !== contract.id; });
    ...
    case 'UPDATE_PAYMENT':
      return {
        ...
        apartments: state.apartments.map(a => {
          if (a.id !== action.payload.apartmentId) return a;
          return { ...a, payments: a.payments.map(p => p.id === action.payload.payment.id ? action.payload.payment : p) };
        }),
      };
    default:
      return state;
